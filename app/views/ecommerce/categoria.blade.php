...
@section('cart')
    <div class="categoria row">
        ...
        <h3>{{$categoria[0]->name}}</h3>
        <div ng-init='products = <?=$products?>' 
            infinite-scroll='loadMore({{$categoria[0]->id}}, @{{products.last_page}})' 
            ...
            <div ng-repeat="product in products.data | orderBy:predicate:reverse" class="producto small-12 medium-3 large-3 columns">
                ...
                <div class="tools">
                    <ngcart-addtocart id="@{{product.id}}" name="@{{product.titulo}}" price="@{{product.costo}}" quantity="1" quantity-max="5" data="item" userpoints="2000"  img="{{url('/')}}/img/productos/@{{product.imagen}}" stock="@{{product.stock}}" userstate="{{ $socio->bloqueado }}" >
